...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const u = await window.storage.get("orcacpu:usuario");
        setUsuario(JSON.parse(u.value));
      } catch {
...
/* ─────────────────────────────────────────────────────────────
   ABA MÃO DE OBRA
───────────────────────────────────────────────────────────── */
function MaoDeObraTab({ etapas }) {
  // Estado para alternar o filtro de insumos consolidado na mesma aba se desejado
  const [filtroTipo, setFiltroTipo] = useState("MO"); // MO, MAT ou EQUIP

  // Consolidação dinâmica dos insumos do orçamento ativo
  const resumoInsumos = useMemo(() => {
    const totais = new Map();

    (etapas || []).forEach((etapa) => {
      (etapa.itens || []).forEach((item) => {
        const qtdCpu = Number(item.quantidade) || 0;
        
        (item.insumos || []).forEach((insumo) => {
          // Normaliza o tipo para evitar falhas por espaços ou letras minúsculas
          let tipoNormalizado = String(insumo.tipo || "").toUpperCase().trim();
          
          // Trata mapeamentos comuns caso venha escrito "MÃO DE OBRA" ou "MATERIAL" por extenso
          if (tipoNormalizado.includes("MÃO") || tipoNormalizado.includes("MAO") || tipoNormalizado === "MO") {
            tipoNormalizado = "MO";
          } else if (tipoNormalizado.includes("MAT") || tipoNormalizado === "MATERIAL") {
            tipoNormalizado = "MAT";
          } else if (tipoNormalizado.includes("EQUIP") || tipoNormalizado === "EQUIPAMENTO") {
            tipoNormalizado = "EQUIP";
          } else {
            tipoNormalizado = "OUTROS";
          }

          // Filtra apenas o tipo selecionado na sub-aba
          if (tipoNormalizado !== filtroTipo) return;

          if (!String(insumo.descricao || "").trim()) return;
const chave = `${String(insumo.descricao).toLowerCase().trim()}_${(insumo.unidade || "un").toLowerCase().trim()}`;
          const coef = Number(insumo.coeficiente) || 0;
          const valorUnit = Number(insumo.valorUnitario) || 0;
          const qtdTotalInsumo = coef * qtdCpu;

          if (totais.has(chave)) {
            const existente = totais.get(chave);
            existente.horas += qtdTotalInsumo; // 'horas' ou quantidade acumulada
            existente.valor += qtdTotalInsumo * valorUnit;
            // Se o valor unitário estava zerado antes e agora veio preenchido, atualiza
            if (existente.valorUnitario === 0 && valorUnit > 0) {
            ...
            totais.set(chave, {
              ...
              unidade: insumo.unidade || "un",
              horas: qtdTotalInsumo, 
              valorUnitario: valorUnit,
              valor: qtdTotalInsumo * valorUnit,
            });
          }
        });
      });
    ...
    return Array.from(totais.values()).sort((a, b) => a.descricao.localeCompare(b.descricao, "pt-BR"));
  }, [etapas, filtroTipo]);

  const totalGeralQuantidades = resumoInsumos.reduce((acc, r) => acc + r.horas, 0);
  const totalGeralValores = resumoInsumos.reduce((acc, r) => acc + r.valor, 0);

  const titulos = {
    MO: { nome: "Mão de Obra", qtdLabel: "Horas Totais" },
    MAT: { nome: "Materiais", qtdLabel: "Qtd Total Consumo" },
    EQUIP: { nome: "Equipamentos", qtdLabel: "Horas/Qtd Equip." }
  };
  ...
  return (
    <div className="bg-white border border-stone-200 shadow-sm rounded-lg overflow-hidden p-5 space-y-4">
      <div className="flex justify-between items-center flex-wrap gap-2">
        <div>
          <h2 className="text-base font-semibold text-stone-800">
            Consolidação Qualitativa do Orçamento
          </h2>
          <p className="text-xs text-stone-500">
            Visualização agrupada de todos os recursos alocados nas composições do projeto ativo.
          </p>
        </div>
        
        {/* Seletor de Tipo para carregar dinamicamente os itens */}
        <div className="flex bg-stone-100 p-1 rounded-lg border border-stone-200">
          <button 
            onClick={() => setFiltroTipo("MO")} 
            className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${filtroTipo === "MO" ? "bg-white text-stone-900 shadow-xs" : "text-stone-500 hover:text-stone-800"}`}
          >
            👷 Mão de Obra
          </button>
          <button 
            onClick={() => setFiltroTipo("MAT")} 
            className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${filtroTipo === "MAT" ? "bg-white text-stone-900 shadow-xs" : "text-stone-500 hover:text-stone-800"}`}
          >
            📦 Materiais
          </button>
          <button 
            onClick={() => setFiltroTipo("EQUIP")} 
            className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${filtroTipo === "EQUIP" ? "bg-white text-stone-900 shadow-xs" : "text-stone-500 hover:text-stone-800"}`}
          >
            🚜 Equipamentos
          </button>
        </div>
      ...
      <div className="border border-stone-200 rounded-lg overflow-hidden">
        {/* Cabeçalho da Grade */}
        <div className="grid grid-cols-12 gap-2 px-4 py-2.5 bg-stone-100 border-b border-stone-200 text-stone-500 font-semibold text-[11px] uppercase tracking-wider">
          <span className="col-span-5">Descrição do Insumo ({titulos[filtroTipo]?.nome})</span>
          <span className="col-span-1 text-center">Und</span>
          <span className="col-span-2 text-right">{titulos[filtroTipo]?.qtdLabel}</span>
          <span className="col-span-2 text-right">Preço Unit. Base</span>
          <span className="col-span-2 text-right">Subtotal Direto</span>
        </div>
        ...
        <div className="divide-y divide-stone-100 max-h-[500px] overflow-y-auto">
          {resumoInsumos.length === 0 ? (
            <div className="p-8 text-center text-stone-400 italic text-xs bg-stone-50/30">
              Nenhum insumo do tipo selecionado foi encontrado nas planilhas do orçamento atual. Verifique se o campo "Tipo" está preenchido corretamente nas composições.
            </div>
          ) : (
            resumoInsumos.map((r, idx) => (
              <div key={idx} className="grid grid-cols-12 gap-2 px-4 py-2 text-xs items-center hover:bg-stone-50/60 transition-colors">
                <span className="col-span-5 font-medium text-stone-800 uppercase truncate">{r.descricao}</span>
                <span className="col-span-1 text-center text-stone-400 text-xs uppercase">{r.unidade}</span>
                <span className="col-span-2 text-right font-mono text-stone-700">
                  {r.horas.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 3 })}
                </span>
                <span className="col-span-2 text-right font-mono text-stone-500">
                  {r.valorUnitario > 0 ? `R$ ${r.valorUnitario.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : <span className="text-stone-300">—</span>}
                </span>
                <span className="col-span-2 text-right font-mono font-semibold text-stone-900">
                  {r.valor > 0 ? `R$ ${r.valor.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : <span className="text-stone-300">—</span>}
                </span>
              </div>
            ))
          )}
        ...
        {/* Linha de Totais da listagem */}
        {resumoInsumos.length > 0 && (
          <div className="grid grid-cols-12 gap-2 px-4 py-2.5 bg-stone-900 text-white text-xs font-semibold uppercase tracking-wider">
            <span className="col-span-6">Total Acumulado ({titulos[filtroTipo]?.nome})</span>
            <span className="col-span-2 text-right font-mono">
              {totalGeralQuantidades.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
            <span className="col-span-2"></span>
            <span className="col-span-2 text-right font-mono">
              R$ {totalGeralValores.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
          </div>
        )}
      </div>
